...
$bg_color = get_theme_mod( 'coachpress_faqs_section_bg_color' );
$bg_image_id = get_theme_mod( 'coachpress_faqs_section_bg_image' );
$bg_image = $bg_image_id ? wp_get_attachment_image_url( $bg_image_id, 'full' ) : '';
$heading_color = get_theme_mod( 'coachpress_faqs_section_heading_color' );
$text_color = get_theme_mod( 'coachpress_faqs_section_text_color' );
$padding = get_theme_mod( 'coachpress_faqs_section_padding', json_encode( [ 'top' => '60px', 'bottom' => '60px' ] ) );
$padding_decoded = json_decode( $padding, true );
$alignment = get_theme_mod('coachpress_faqs_section_alignment', 'center');

?>

<section id="faqs" class="faqs-section" style="background-color: <?php echo esc_attr( $bg_color ); ?>; <?php if ( $bg_image ) : ?> background-image: url(<?php echo esc_url( $bg_image ); ?>); <?php endif; ?> padding-top: <?php echo esc_attr( $padding_decoded['top'] ); ?>; padding-bottom: <?php echo esc_attr( $padding_decoded['bottom'] ); ?>; text-align: <?php echo esc_attr($alignment); ?>;">
    <div class="container">
        <h2 style="color: <?php echo esc_attr( $heading_color ); ?>;" data-aos="fade-up"><?php echo esc_html( get_theme_mod( 'coachpress_faqs_section_title', '' ) ); ?></h2>
